...
import { createChatSession, geminiPrompt, parseGeminiJSON } from '../lib/gemini'
      ...
      {isAI && (
        ...
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
            stroke="var(--teal)" strokeWidth="2.2" strokeLinecap="round">
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        <div
          ...
          style={{ background: 'var(--teal-dim)', border: '1px solid var(--border-2)' }}
        >
          <span style={{ fontSize: 9, color: 'var(--teal)', fontFamily: 'Syne, sans-serif', fontWeight: 700 }}>
            {index + 1}
          </span>
        </div>
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        ...
        <span className="font-syne font-bold text-base shrink-0" style={{ color }}>
...
function Spinner({ label, sub }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-4">
      ...
      {sub && <p className="text-xs text-center" style={{ color: 'var(--text-3)', maxWidth: 220 }}>{sub}</p>}
    </div>
  )
}
...
export default function Phase1Diagnose() {
  ...
  const { setCandidates, setQuestions: setCtxQuestions, profile } = useDiagnosis()
...
`.trim()
    ...
    try {
      ...
      const hasStomach = fullTranscript.toLowerCase().includes('stomach') || fullTranscript.toLowerCase().includes('nausea')
  ...
  const runPrediction = async () => {
    ...
    const scores = computeScores(quizData.questions, answers, quizData.diseases || [])
...
`.trim()
  ...
  const progressPct =
    phase === 'chat'                        ? 10 :
    phase === 'quiz' && quizLoading         ? 45 :
    phase === 'quiz'                        ? 45 + Math.round((answeredN / Math.max(totalQ, 1)) * 45) :
    phase === 'predicting'                  ? 95 :
    phase === 'result' || phase === 'emergency' ? 100 : 0

  const progressColor =
    phase === 'emergency' ? 'var(--red)'   :
    phase === 'result'    ? 'var(--green)' :
    phase === 'quiz'      ? 'var(--amber)' : 'var(--teal)'
  ...
  return (
    <div className="page-shell">
      <Navbar
        ...
        subtitle={
          phase === 'chat'       ? 'Describe your symptoms' :
          phase === 'quiz'       ? `Quiz · ${answeredN}/${totalQ} answered` :
          phase === 'predicting' ? 'Running prediction…' :
          phase === 'result'     ? 'Prediction ready' :
          'Emergency detected'
        }
        backTo="/onboarding"
        rightSlot={
          <span className={`badge ${
            phase === 'emergency' ? 'badge-red'   :
            phase === 'result'    ? 'badge-green' :
            phase === 'quiz'      ? 'badge-amber' : 'badge-teal'
          }`}>
            {phase === 'chat' ? 'Chat' : phase === 'quiz' ? 'Quiz' :
             phase === 'predicting' ? 'AI' : phase === 'result' ? 'Done' : 'Urgent'}
          </span>
        }
      />
      ...
      {phase === 'emergency' && (
        <div className="flex-1 overflow-y-auto px-5 py-6">
          <div className="anim-fade-up p-5 rounded-2xl mb-4"
            style={{ background: 'var(--red-dim)', border: '2px solid var(--red)' }}>
            <div className="w-14 h-14 rounded-full flex items-center justify-center mb-4"
              style={{ background: 'var(--red)', animation: 'pulseRed 1.2s infinite' }}>
              <svg width="26" height="26" viewBox="0 0 24 24" fill="white">
                <path d="M12 2L1 21h22L12 2zm0 3.5L20.5 19h-17L12 5.5zM11 10v4h2v-4h-2zm0 6v2h2v-2h-2z"/>
              </svg>
            </div>
            <h2 className="font-syne font-bold text-xl mb-2" style={{ color: 'var(--red)' }}>
              Emergency detected
            </h2>
            ...
            <p className="text-sm leading-relaxed mb-5" style={{ color: 'var(--text-2)' }}>
              Your symptoms suggest a potentially life-threatening condition. Please call emergency services immediately. Do not drive yourself.
            </p>
            <a href="tel:108"
      ...
      {phase === 'result' && result && (
        <div className="flex-1 overflow-y-auto px-5 py-6">
          <div className="anim-fade-up mb-2">
            <span className={`badge ${result.phase === 'confident' ? 'badge-green' : 'badge-amber'} inline-block mb-3`}>
              {result.phase === 'confident' ? 'AI is confident' : 'Closest matches'}
            </span>
            <h2 className="font-syne font-bold text-2xl mb-1" style={{ color: 'var(--text-1)' }}>
          ...
          {result.phase === 'unsure' && (
            <div className="p-4 rounded-xl mb-5"
              style={{ background: 'var(--amber-dim)', border: '1px solid rgba(245,158,11,.3)', borderLeft: '3px solid var(--amber)', borderRadius: '0 12px 12px 12px' }}>
              <p className="text-sm font-medium mb-1" style={{ color: 'var(--amber)' }}>
                Not satisfied?
              </p>
              <p className="text-xs" style={{ color: 'var(--text-2)' }}>
                Retake the quiz for better accuracy, or describe your symptoms in more detail.
              </p>
            </div>
          )}

          <div className="flex flex-col gap-3">
            <button className="btn-primary" onClick={() => navigate('/contain')}>
          ...
          {quizLoading ? (
            ...
            <>
              ...
              <div className="p-3 rounded-xl mb-5"
                style={{ background: 'var(--teal-dim)', border: '1px solid var(--border-2)' }}>
                <p className="text-xs font-medium mb-0.5" style={{ color: 'var(--teal)' }}>
                  Personalised quiz · powered by Gemini
                </p>
                <p className="text-xs" style={{ color: 'var(--text-2)' }}>
                  {quizData.questions.length} targeted questions generated from your description
                </p>
              </div>
        ...
        <div className="flex flex-col" style={{ flex: 1, minHeight: 0 }}>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-4">
            ...
            {chatReady && !chatLoading && (
              <div className="p-4 rounded-xl anim-fade-up"
                style={{ background: 'var(--green-dim)', border: '1px solid rgba(16,185,129,.3)' }}>
                <p className="text-sm font-medium mb-1"
                  style={{ color: 'var(--green)', fontFamily: 'Syne, sans-serif' }}>
                  Got it — ready to generate your quiz
                </p>
                <p className="text-xs mb-3" style={{ color: 'var(--text-2)' }}>
                  Gemini has understood your symptoms and will now generate a personalised diagnostic quiz.
                </p>
                <button
                  onClick={generateQuiz}
                  className="text-sm font-semibold px-4 py-2 rounded-xl"
                  style={{ background: 'var(--green)', color: '#fff', border: 'none', cursor: 'pointer', fontFamily: 'Syne, sans-serif' }}>
            ...
            {chatReady ? (
              <div className="flex items-center gap-2 p-3 rounded-xl"
                ...
                <p className="text-xs" style={{ color: 'var(--text-3)' }}>
                  Consultation complete · click "Generate my personalised quiz" above
                </p>
              </div>
            ) : (
              <div className="flex gap-3 items-end">
                ...
                <button
                  ...
                  style={{
                    width: 44, height: 44,
                    background: inputText.trim() && !chatLoading ? 'var(--teal)' : 'var(--navy-light)',
                    border: '1px solid var(--border)',
                    cursor: inputText.trim() && !chatLoading ? 'pointer' : 'default',
                  }}
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                    stroke={inputText.trim() && !chatLoading ? 'var(--navy)' : 'var(--text-3)'}
      ...
      {phase === 'quiz' && !quizLoading && quizData && (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs" style={{ color: allDone ? 'var(--green)' : 'var(--text-3)' }}>
              {allDone ? 'All answered ✓' : `${totalQ - answeredN} remaining`}
            </span>
          </div>
          <div className="h-1 rounded-full overflow-hidden mb-3" style={{ background: 'var(--navy-light)' }}>
            <div style={{
              height: '100%',
          ...
          <button className="btn-primary" onClick={runPrediction} disabled={!allDone}>
            {allDone ? 'Predict my condition →' : `Answer all ${totalQ} questions to predict`}
          </button>
        </div>
      )}
...
function buildFallbackQuiz(hasFever, hasHead, hasStomach) {
  ...
  if (hasFever) {
    questions.push({
      ...
      options: [
        { label: 'Very high (above 103°F)', value: 'high',   points: { Dengue: 25, Malaria: 25, Typhoid: 20 } },
        { label: 'Moderate (100–103°F)',    value: 'mod',    points: { Influenza: 20, Typhoid: 10 } },
        { label: 'Low-grade (below 100°F)', value: 'low',    points: { CommonCold: 15 } },
      ],
    })
    questions.push({
      id: 'q_fever_pattern',
      ...
      options: [
        { label: 'Continuous, never goes away',  value: 'cont',  points: { Typhoid: 30 } },
        { label: 'Comes and goes in cycles',      value: 'cycl',  points: { Malaria: 40 } },
        { label: 'Sudden spike then drops',       value: 'spike', points: { Dengue: 30, Influenza: 20 } },
      ],
    })
  }
  if (hasHead) {
    questions.push({
      ...
      options: [
        { label: 'Throbbing on one side',  value: 'throb', points: { Migraine: 40 } },
        { label: 'Pressure all over',      value: 'press', points: { Influenza: 20, Dengue: 15 } },
        { label: 'Stiff neck with pain',   value: 'stiff', points: { Meningitis: 45 } },
      ],
    })
  }
  if (hasStomach) {
    questions.push({
      ...
      options: [
        { label: 'Lower right side',  value: 'lr',  points: { Appendicitis: 45 } },
        { label: 'All over stomach',  value: 'all', points: { Gastroenteritis: 30 } },
  ...
  questions.push({
    ...
    options: [
      { label: 'Just today',       value: 'today', points: { Influenza: 10, CommonCold: 10 } },
      { label: '2–5 days',         value: 'days',  points: { Dengue: 15, Typhoid: 10 } },
      { label: '1–2 weeks',        value: 'week',  points: { Typhoid: 20, Tuberculosis: 10 } },
      { label: 'More than 2 weeks', value: 'long', points: { Tuberculosis: 20, Hepatitis: 15 } },
    ],
  })
  return {
    questions,
    diseases: ['Dengue', 'Malaria', 'Typhoid', 'Influenza', 'CommonCold',
               'Migraine', 'Meningitis', 'Appendicitis', 'Gastroenteritis',
               'Tuberculosis', 'Hepatitis', 'Gastritis'],
  }
}
...
function buildFallbackResult(scores, diseases) {
  ...
  if (!candidates.length) {
    candidates.push({ disease: 'Undetermined', confidence: 40, reason: 'Insufficient data to predict' })
  }
